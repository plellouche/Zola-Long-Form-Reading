...
import { getArticles, getSourceBySlug } from '@/lib/api';
...
export default function SourceScreen() {
  ...
  const articlesQuery = useQuery({
    queryKey: ['articles', 'source', slug],
    queryFn: () => getArticles({ source_slug: slug, sort: 'newest', limit: 30 }),
    enabled: !!slug,
    staleTime: 60 * 1000,
  });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {articlesQuery.isLoading ? (
        ...
        <EmptyState icon="rss" title="No articles" subtitle="No articles from this source yet" />
      ) : (
